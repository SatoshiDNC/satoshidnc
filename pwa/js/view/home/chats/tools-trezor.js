...
import { trezorConnect, trezorPing, trezorRestore, trezorGetAddress, trezorGetNostrPubKey, trezorGetPassword, trezorSign, trezorWipe, trezorClose } from '../../../trezor.js'
          ...
          case GEN_PW:
            {
              ...
              window.crypto.subtle.digest('SHA-256', new TextEncoder().encode(text)).then(bytes => {
                ...
                trezorGetPassword(index).then(r => {
                  ...
                  // })
                  clearSelection()
                  item.passwd = (new Uint8Array(r.nodeType.publicKey)).map(v => (v > 15?'':'0') + v.toString(16)).join('')
                  item.subtitle = '********************************************'
                  v.setRenderFlag(true)
                }).catch(handleError)    
              })
