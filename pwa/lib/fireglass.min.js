...
c.focus(),c.select())}function xa(a){if(a!=ca){var c=0;0<ca&&(c=a-ca);ca=a;const b=[];for(;a=C.pop();)b.push(a);if(ia)ia=!1,k.viewport(0,0,w.width,w.height),mat4.identity(O),mat4.translate(O,O,[-1,1,0]),mat4.scale(O,O,[2/w.width,-2/w.height,1]),x&&(a=new m.LayoutState(w.width,w.height),x.layoutAll(a));else for(;a=b.pop();)a.relayout();ja&&(--ja||x.setRenderFlag(!0));ka&&x.setRenderFlag(!0);if(x&&(x.needsRender||x.childRender)){a=0;for(var [,d]of Object.entries(u))d.dx=(d.x-d.px)/c,d.px=d.x,d.dy=(d.y-
d.py)/c,d.py=d.y,a+=Math.sqrt(d.dx*d.dx+d.dy*d.dy)/ya;la&&la.setValueAtTime(a,G.currentTime);x.renderAll();if(za){k.viewport(0,0,w.width,w.height);k.disable(k.SCISSOR_TEST);k.useProgram(A);k.bindBuffer(k.ARRAY_BUFFER,ma);c=k.getAttribLocation(A,"aVertexPosition");k.vertexAttribPointer(c,2,k.FLOAT,!1,0,0);k.enableVertexAttribArray(c);k.enable(k.BLEND);k.uniform4fv(k.getUniformLocation(A,"uVertexColor"),new Float32Array([1,1,1,1]));k.uniformMatrix4fv(k.getUniformLocation(A,"uProjectionMatrix"),!1,O);
...
k.compileShader(f);if(k.getShaderParameter(f,k.COMPILE_STATUS)){var b=k.createShader(k.FRAGMENT_SHADER);k.shaderSource(b,"\n        varying lowp vec4 v_Color;\n        void main() {\n          gl_FragColor = v_Color;\n        }\n\t");k.compileShader(b);if(k.getShaderParameter(b,k.COMPILE_STATUS)&&(A=k.createProgram(),k.attachShader(A,f),k.attachShader(A,b),k.linkProgram(A),k.getProgramParameter(A,k.LINK_STATUS))){ma=k.createBuffer();k.bindBuffer(k.ARRAY_BUFFER,ma);k.vertexAttribPointer(k.getAttribLocation(A,
...
T;l&&l.pasteFunc&&l.pasteFunc.call(l,h)}),document.addEventListener("keydown",function(h){var l=T;l&&l.keydownFunc&&l.keydownFunc.call(l,h)}))}}}}let m;var w,oa,pa,ba,aa,fa,ha,k;const z={},E={};var G,ra,P,la,ma,ya,D,ia;const C=[];const O=mat4.create();var qa=0,A,T,J,sa,ta,ua,va,wa;class Da{constructor(a,c){this.x=a;this.y=c;this.hits=[]}sortHits(){for(var a=[];0<this.hits.length;){var c=0,d=this.hits[0].dist,f,b=this.hits[0].gad.z?this.hits[0].gad.z:0,g;for(f=0;f<this.hits.length&&(g=this.hits[f].gad.z?
...
b[(e+3)%b.length]));h<g&&(g=h)}return g}}class Ja extends X{constructor(a){super(a);this.dragBeginFunc=function(c){this.tempX=c.ox-this.viewport.x;this.tempY=c.oy-this.viewport.y};this.dragMoveFunc=function(c){var d=this.viewport.parent,f="h"==d.state,b=f?c.y-this.tempY-d.a.y:c.x-this.tempX-d.a.x;d.ratio=Math.max(0,Math.min(1,b/(b+(f?d.y+d.h-(c.y-this.tempY+this.viewport.h):d.x+d.w-(c.x-this.tempX+this.viewport.w)))));d.setPoint||(d.setPoint={});d.setPoint[d.state]=d.ratio;-1===C.indexOf(this.viewport.parent)&&
C.push(this.viewport.parent)};this.dragEndFunc=function(c){delete this.tempX;delete this.tempY}}layout(){var a=this.viewport,c=a.getScale(),d=a.parent.size,f="h"==a.parent.state;var b=f?Math.max(1,a.w)/c:d;a=f?d:Math.max(1,a.h)/c;this.actionFlags=f?m.GAF_DRAGGABLE_UPDOWN:m.GAF_DRAGGABLE_LEFTRIGHT;this.convexHull=this.computeHull([0,0,0,a,b,a,b,0]);this.extendedHulls={};this.boundingBoxes={}}}class Ka extends X{getTargetMat(){return this.targetGad?this.targetGad.mat:this.targetView.userMat}constructor(a){super(a);
...
m.GAF_BACKNAV&&a.clickFunc&&a.clickFunc.call(a,void 0)},View:U,DividerView:Ea,SliceView:Ga,PagesView:Ha,PopupView:Ia,OverlayView:Fa,ViewPicker:Ca,Gadget:X,MatrixGadget:Ka,SwipeGadget:La,MiddleDividerGadget:Ja,GAF_CLICKABLE:1,GAF_HOLDABLE:2,GAF_CONTEXTMENU:4,GAF_STRETCHABLE:8,GAF_DRAGGABLE_UPDOWN:16,GAF_DRAGGABLE_LEFTRIGHT:32,GAF_DRAGGABLE:48,GAF_SWIPEABLE_UPDOWN:64,GAF_SWIPEABLE_LEFTRIGHT:128,GAF_SWIPEABLE:192,GAF_SCROLLABLE_UPDOWN:256,GAF_SCROLLABLE_LEFTRIGHT:512,GAF_SCROLLABLE:768,GAF_SCALABLE:1024,
GAF_ROTATABLE:2048,GAF_PINCHABLE:3072,GAF_NUMINPUT:4096,GAF_TEXTINPUT:8192,GAF_GONEXT:16384,GAF_BACKNAV:32768,GAF_ALL:3901,HitList:Da,LayoutState:I}}();
