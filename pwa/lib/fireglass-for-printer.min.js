...
for(const [,g]of Object.entries(u))mat4.identity(c),mat4.translate(c,c,[12,72,0]),mat4.scale(c,c,[46*g.y/D/1E3,6,0]),k.uniformMatrix4fv(k.getUniformLocation(C,"uModelViewMatrix"),!1,c),k.drawArrays(k.LINE_LOOP,z.unitSquare,F.unitSquare)}}}window.requestAnimationFrame(xa)}function V(){ja=10;var a;if(oa&&pa){var c=oa;var d=pa;var f=a=1;w.width=c;w.height=d;w.style.width=c;w.style.height=d}else c=window.innerWidth,d=window.innerHeight,a=window.devicePixelRatio,f=window.visualViewport.scale,w.width=c*
f*a,w.height=d*f*a,w.style.width=c*f,w.style.height=d*f;ya=Math.sqrt(w.width*w.width+w.height*w.height);D=f*a;ia=!0;x.resizeFunc&&x.resizeFunc.call(x)}function K(a,c){function d(n,q){for(const r of n.hitList.hits)r.gad!==q&&r.gad.ownedBy==g&&(r.gad.gestureState+="-disowned",r.gad.viewport.setRenderFlag(!0),delete r.gad.ownedBy)}function f(n){const q=n.x-n.ox;n=n.y-n.oy;return q*q>n*n?0<q?"right":"left":0<n?"down":"up"}if("touches"==c&&a.touches&&0==a.touches.length){a=[];for(var [,b]of Object.entries(u))b.tempIsActive=
