...
0;var h=Date.now();0!=this.prevTime&&(e=h-this.prevTime);this.prevTime=h;17<e&&(e=17);a.remainingWidth*this.threshold>a.remainingHeight*(1-this.threshold)+2&&(this.stateNew=this.wideState);a.remainingHeight*this.threshold>a.remainingWidth*(1-this.threshold)+2&&(this.stateNew=this.tallState);if(this.stateNew!=this.state||0<this.stateProg)this.stateNew!=this.state?(this.stateProg+=.008*e,1<=this.stateProg&&(this.stateProg=1,this.state=this.stateNew),-1===B.indexOf(this)&&B.push(this)):(this.stateProg-=
.008*e,0>this.stateProg&&(this.stateProg=0),0<this.stateProg&&-1===B.indexOf(this)&&B.push(this))}this.layoutFunc&&this.state!=g&&0<this.w&&0<this.h&&this.layoutFunc.call(this);e="t"==this.state||"b"==this.state?b/a.remainingWidth:f/a.remainingHeight;g=1-this.stateProg*(1-e);e=1-this.stateProg*(0+e);"l"==this.state?(c.remainingWidth=b,c.remainingHeight=~~(a.remainingHeight*g),c.remainingX=a.remainingX,d.remainingWidth=a.remainingWidth-b,d.remainingHeight=~~(a.remainingHeight*e),d.remainingX=a.remainingX+
...
0!=this.prevTime&&(e=h-this.prevTime);this.prevTime=h;17<e&&(e=17);a.remainingWidth*this.threshold>a.remainingHeight*(1-this.threshold)+2&&(this.stateNew=this.wideState);a.remainingHeight*this.threshold>a.remainingWidth*(1-this.threshold)+2&&(this.stateNew=this.tallState);if(this.stateNew!=this.state||0<this.stateProg)this.stateNew!=this.state?(this.stateProg+=.008*e,1<=this.stateProg&&(this.stateProg=1,this.state=this.stateNew),-1===B.indexOf(this)&&B.push(this)):(this.stateProg-=.008*e,0>this.stateProg&&
(this.stateProg=0),0<this.stateProg&&-1===B.indexOf(this)&&B.push(this))}this.layoutFunc&&this.state!=g&&0<this.w&&0<this.h&&this.layoutFunc.call(this);e="t"==this.state||"b"==this.state?b/a.remainingWidth:f/a.remainingHeight;g=1-this.stateProg*(1-e);e=1-this.stateProg*(0+e);"l"==this.state?(c.remainingWidth=b,c.remainingHeight=~~(a.remainingHeight*g),c.remainingX=a.remainingX,d.remainingWidth=a.remainingWidth-b,d.remainingHeight=~~(a.remainingHeight*e),d.remainingX=a.remainingX+b,"t"==this.tallState?
...
dist:c})}}}}autoHull(){this.convexHull=this.computeHull([0,0,this.w,0,this.w,this.h,0,this.h]);this.extendedHulls={};this.boundingBoxes={}}computeHull(a){function c(t,v,A,E,L,M){return(A-t)*(M-v)-(E-v)*(L-t)}function d(t){return t*t}function f(t,v,A,E,L,M){var H=d(A-L)+d(E-M);if(0==H)return d(t-A)+d(v-E);H=Math.max(0,Math.min(1,((t-A)*(L-A)+(v-E)*(M-E))/H));E+=H*(M-E);return d(t-(A+H*(L-A)))+d(v-E)}function b(t,v,A,E,L,M){if(!(2>v.length)){for(var H=0,R=0,y=0;y<~~(v.length/2);y++){var S=Math.sqrt(f(v[2*
y],v[2*y+1],A,E,L,M));S>R&&(R=S,H=y)}R=v[2*H];S=v[2*H+1];v.splice(2*H,2);for(y=0;y<~~(t.length/2);y++)if(t[2*y]==A&&t[2*y+1]==E&&t[(2*y+2)%t.length]==L&&t[(2*y+3)%t.length]==M||t[2*y]==L&&t[2*y+1]==M&&t[(2*y+2)%t.length]==A&&t[(2*y+3)%t.length]==E){t.splice(2*y+2,0,R,S);break}H=[];var Y=[];for(y=0;y<~~(v.length/2);y++)0>c(A,E,R,S,v[2*y],v[2*y+1])&&(H[H.length]=v[2*y],H[H.length]=v[2*y+1]),0>c(R,S,L,M,v[2*y],v[2*y+1])&&(Y[Y.length]=v[2*y],Y[Y.length]=v[2*y+1]);b(t,H,A,E,R,S);b(t,Y,R,S,L,M)}}for(var g=
...
c,b[0],b[1],b[2],b[3])),e=2;e<b.length;e+=2){var h=Math.sqrt(f(a,c,b[e+0],b[e+1],b[(e+2)%b.length],b[(e+3)%b.length]));h<g&&(g=h)}return g}}class Ja extends X{constructor(a){super(a);this.dragBeginFunc=function(c){this.tempX=c.ox-this.viewport.x;this.tempY=c.oy-this.viewport.y};this.dragMoveFunc=function(c){var d=this.viewport.parent,f="h"==d.state,b=f?c.y-this.tempY-d.a.y:c.x-this.tempX-d.a.x;d.ratio=Math.max(0,Math.min(1,b/(b+(f?d.y+d.h-(c.y-this.tempY+this.viewport.h):d.x+d.w-(c.x-this.tempX+this.viewport.w)))));
d.setPoint||(d.setPoint={});d.setPoint[d.state]=d.ratio;-1===B.indexOf(this.viewport.parent)&&B.push(this.viewport.parent)};this.dragEndFunc=function(c){delete this.tempX;delete this.tempY}}layout(){var a=this.viewport,c=a.getScale(),d=a.parent.size,f="h"==a.parent.state;var b=f?Math.max(1,a.w)/c:d;a=f?d:Math.max(1,a.h)/c;this.actionFlags=f?m.GAF_DRAGGABLE_UPDOWN:m.GAF_DRAGGABLE_LEFTRIGHT;this.convexHull=this.computeHull([0,0,0,a,b,a,b,0]);this.extendedHulls={};this.boundingBoxes={}}}class Ka extends X{getTargetMat(){return this.targetGad?
...
return m={initialize:Aa,start:Ba,pushRoot:function(a){Z.push(x);x=a;x.queueLayout();V()},peekRoot:function(){return Z[Z.length-1]},popRoot:function(){x=Z.pop();x.queueLayout();V()},getRoot:function(){return x},setRoot:function(a){Z=[];x=a;x.queueLayout();V()},setDebugFlags:function(a,c){za=!!a;ka=!!c},setKeystrokeSound:function(a){W=a},getContext:function(){return k},getPointerRadius:N,isMouseEnabled:function(){return qa},getInputGad:function(){return T},beginInput:ea,endInput:da,setImage:function(a){if(!J){var c=
...
G.currentTime+.001);break;default:P.setValueAtTime(1,G.currentTime),P.setValueAtTime(0,G.currentTime+.1)}},backAction:function(){var a=new m.HitList(x.w/2,x.h/2);x.getHits(a,Math.sqrt((x.w/2)**2+(x.h/2)**2));a.sortHits();for(const c of a.hits)a=c.gad,!a.ownedBy&&a.actionFlags&m.GAF_BACKNAV&&a.clickFunc&&a.clickFunc.call(a,void 0)},View:U,DividerView:Ea,SliceView:Ga,PagesView:Ha,PopupView:Ia,OverlayView:Fa,ViewPicker:Ca,Gadget:X,MatrixGadget:Ka,SwipeGadget:La,MiddleDividerGadget:Ja,GAF_CLICKABLE:1,
GAF_HOLDABLE:2,GAF_CONTEXTMENU:4,GAF_STRETCHABLE:8,GAF_DRAGGABLE_UPDOWN:16,GAF_DRAGGABLE_LEFTRIGHT:32,GAF_DRAGGABLE:48,GAF_SWIPEABLE_UPDOWN:64,GAF_SWIPEABLE_LEFTRIGHT:128,GAF_SWIPEABLE:192,GAF_SCROLLABLE_UPDOWN:256,GAF_SCROLLABLE_LEFTRIGHT:512,GAF_SCROLLABLE:768,GAF_SCALABLE:1024,GAF_ROTATABLE:2048,GAF_PINCHABLE:3072,GAF_NUMINPUT:4096,GAF_TEXTINPUT:8192,GAF_GONEXT:16384,GAF_BACKNAV:32768,GAF_ALL:3901,HitList:Da,LayoutState:I}}();
